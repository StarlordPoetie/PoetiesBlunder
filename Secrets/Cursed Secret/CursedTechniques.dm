...
/obj/Skills/proc/validateCursedDomainSureHit(mob/p, trait)
	if(!p)
		return 0
	return p.canUseCursedEnergyDomainSureHit(trait)
...
/mob/proc/HasSixEyes()
	return cursedEnergySixEyesActive()
...
/mob/proc/canUseCursedEnergyDomainSureHit(trait)
	if(!hasCursedEnergy())
		src << "Only Cursed Energy users can invoke this sure-hit."
		return 0
	if(cursedEnergyTrait != trait)
		src << "This sure-hit does not match your Cursed Energy trait."
		return 0
	if(!hasActiveCursedEnergyDomain())
		src << "You must have an active Cursed Energy Domain Expansion to use this sure-hit."
		return 0
	return 1
...
/mob/proc/finishCursedEnergyDomainSureHit(obj/Skills/used)
	var/obj/Skills/Buffs/SlotlessBuffs/Domain_Expansion/Cursed_Energy/D = locate() in src
	collapseCursedEnergyDomainSureHit()
	if(D)
		D.Cooldown(p=src)
	if(used)
		used.Cooldown(p=src)
