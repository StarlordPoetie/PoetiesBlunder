...
/mob/proc/OpenSecretSagaDatabase()
	...
	html += "<table><tr><th>#</th><th>Name</th><th>Key / CKEY</th><th>Computer ID</th><th>Type</th><th>Secret</th><th>Secret Tier</th><th>Secret Last Tier Up</th><th>Secret Datum</th><th>Saga</th><th>Saga Tier</th><th>Saga Last Tier Up</th><th>Cursed Energy Details</th><th>Actions</th></tr>"
	var/row = 0
	for(var/mob/P in players)
		if(!P.Secret && !P.Saga)
			continue
		...
		if(P.Secret)
			actions += "<a class='action danger' href='byond://?src=\ref[usr];action=RemoveSecretSaga;target=\ref[P];remove=Secret'>Remove Secret</a>"
		if(P.Saga)
			actions += "<a class='action danger' href='byond://?src=\ref[usr];action=RemoveSecretSaga;target=\ref[P];remove=Saga'>Remove Saga</a>"
		if(P.Secret && P.Saga)
			actions += "<a class='action danger' href='byond://?src=\ref[usr];action=RemoveSecretSaga;target=\ref[P];remove=Both'>Remove Both</a>"
		var/actionText = actions.len ? jointext(actions, "<br>") : "<span class='none'>None</span>"
		html += "<tr class='power'><td>[row]</td><td>[AdminPowerAuditValue(P.name)]</td><td>[keyText]</td><td>[AdminPowerAuditValue(P.client ? P.client.computer_id : null)]</td><td>[powerType]</td><td>[secretName]</td><td>[secretTier]</td><td>[secretLastTierUp]</td><td>[secretType]</td><td>[sagaName]</td><td>[sagaTier]</td><td>[sagaLastTierUp]</td><td>[ceText]</td><td>[actionText]</td></tr>"
	if(!row)
		html += "<tr class='empty'><td colspan='14'>No online players currently have a Secret or Saga.</td></tr>"
	html += "</table>"
...
/mob/proc/AdminSecretSagaRemoveFromLink(mob/Players/P, var/choice)
	if(!Admin || Admin < 3)
		return
	if(!P || !(choice in list("Secret", "Saga", "Both")))
		return
	if(choice == "Secret" && !P.Secret)
		usr << "[P] does not currently have a Secret."
		return
	if(choice == "Saga" && !P.Saga)
		usr << "[P] does not currently have a Saga."
		return
	if(choice == "Both" && !P.Secret && !P.Saga)
